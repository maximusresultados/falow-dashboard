...
export async function syncCompany(companyId) {
  ...
  if (!co) return { error: "company_not_found" };
  ...
  if (!apiToken) return { error: "no_api_token" };
  ...
  for (const panel of panels) {
    ...
    if (syncResult?.error) throw new Error(`admin_sync_panel: ${syncResult.error}`);
